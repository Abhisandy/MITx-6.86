...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        vec_of_relu = np.vectorize(rectified_linear_unit)
        hidden_layer_activation =  vec_of_relu(hidden_layer_weighted_input)# TODO (3 by 1 matrix)

        output =  self.hidden_to_output_weights.dot(hidden_layer_activation)# TODO
        #vec_out = np.vectorize(output_layer_activation)
        ...
        vec_output_layer = np.vectorize(output_layer_activation_derivative)
        hidden_layer_error =   np.multiply(vec_output_layer(activated_output),self.hidden_to_output_weights.transpose())*output_layer_error# TODO (3 by 1 matrix)
        vec_relu = np.vectorize(rectified_linear_unit_derivative)
        bias_gradients = np.multiply(hidden_layer_error, vec_relu(hidden_layer_weighted_input))# TODO
